...
function Create() {
  ...
  const validate = (form) => {
    ...
    if (!/^[0-9/]*$/.test(form.released)) {
      updatedErrors.released = 'Formato de fecha inválido. Utilice solo números y /';
    } else {
      ...
      if (form.released.length === 8) {
        if (!/^(0[1-9]|[12]\d|3[01])\/(0[1-9]|1[0-2])\/\d{2}$/.test(form.released)) {
          updatedErrors.released = 'Formato de fecha inválido. Utilice yyyy/mm/dd';
        } else {
          updatedErrors.released = '';
        }
      }
  ...
  const hasErrors = () => {
    ...
    return Object.values(errors).some((error) => error !== '') || noGenresSelected;
  };
  ...
  return (
    <div className='formWrapper'>
      <Link to='/home' className='estiloLinks'>  Back Home</Link>
      <form onSubmit={submitHandler}>
        ...
        <button type='submit' disabled={!Object.values(form).every(value => value !== '') || hasErrors()}> SUBMIT </button>
      </form>
    </div>
  );
}
