...
const getByIdDB = async (id, source) => {
    ...
    if (source === 'api') {
        game = (await axios.get(`https://api.rawg.io/api/games/${id}?key=${API_KEY}`)).data
...
const getGamesByName = async (name) => {
    ...
    for (let i = 1; i < 6; i++) {
        let allGamesFromApiFullInfo = (await axios.get(`https://api.rawg.io/api/games?key=${API_KEY}&page=${i}`)).data.results;
        let allGamesFromApiFiltered = infoFiltered(allGamesFromApiFullInfo);
        gamesFromApi.push(allGamesFromApiFiltered);
    }
    let allGames = [];
    gamesFromApi.map(el => {allGames = allGames.concat(el)});

    
    const gamesByNameApi = allGames.filter(game => game.name.toLowerCase().includes(name.toLowerCase()));

    if (gamesByNameDB.length === 0 && gamesByNameApi.length === 0) throw new Error (`No se encontraron juegos que contengan ${name} en su nombre`)
